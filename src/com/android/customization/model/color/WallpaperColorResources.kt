...
class WallpaperColorResources {
    ...
    constructor(wallpaperColors: WallpaperColors) {
        ...
        val color = ((colorScheme.accent1.values.first()) as Srgb).toLinear().toOklab().toOklch()
        val fromInt = if (color.chroma < 5.0f) Srgb(GOOGLE_BLUE).toLinear().toOklab().toOklch() else color
        val hue = fromInt.hue.toFloat()
        val chroma = fromInt.chroma.toFloat()
    ...
    companion object {
        ...
        private const val GOOGLE_BLUE = 0x1B6EF3.toInt()
